    ...
    >
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center bg-white rounded-2xl border border-slate-200 shadow-md p-3">
        ...
        <button
          ...
          className="hidden sm:flex items-center justify-center w-8 h-8 rounded-full bg-slate-100 hover:bg-blue-100 text-slate-400 hover:text-blue-600 border border-slate-200 hover:border-blue-300 shrink-0 transition-all duration-150 hover:shadow-sm active:scale-90 group"
        >
          <svg
            className={`w-4 h-4 transition-transform duration-300 ${isSwapping ? 'rotate-180' : ''}`}
        ...
        {/* モバイル: 入れ替えボタン（横並びのとき中央寄せ） */}
        <div className="flex sm:hidden justify-center">
          <button
            id="swap-stations-button-mobile"
            type="button"
            onClick={handleSwap}
            aria-label="出発地と目的地を入れ替え"
            className="flex items-center justify-center w-8 h-8 rounded-full bg-slate-100 hover:bg-blue-100 text-slate-400 hover:text-blue-600 border border-slate-200 hover:border-blue-300 transition-all duration-150 hover:shadow-sm active:scale-90"
          >
            <svg
              className={`w-4 h-4 transition-transform duration-300 ${isSwapping ? 'rotate-180' : ''}`}
              fill="none" viewBox="0 0 24 24" stroke="currentColor"
